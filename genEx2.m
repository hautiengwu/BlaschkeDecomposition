...
for iter = 1:15
	...
	SER1NoC(iter) = norm(real(B_prod(1,:) .* Low(2, :))-real(y0(1,:)))/norm(real(y0(1,:))) ;
	SER2NoC(iter) = norm(real(B_prod(2,:) .* Low(3, :))-real(y0(2,:)))/norm(real(y0(2,:))) ;
	SER1WithC(iter) = norm(real(B_prodC(1,:) .* LowC(2, :))-real(y0(1,:)))/norm(real(y0(1,:))) ;
	SER2WithC(iter) = norm(real(B_prodC(2,:) .* LowC(3, :))-real(y0(2,:)))/norm(real(y0(2,:))) ;


	SERphi1NoC(iter) = norm(IFSST(1,:)-if0(1,:))/norm(if0(1,:)) ;
	SERphi2NoC(iter) = norm(IFSST(2,:)-if0(2,:))/norm(if0(2,:)) ;
	SERphi1WithC(iter) = norm(IFSSTC(1,:)-if0(1,:))/norm(if0(1,:)) ;
	SERphi2WithC(iter) = norm(IFSSTC(2,:)-if0(2,:))/norm(if0(2,:)) ;
